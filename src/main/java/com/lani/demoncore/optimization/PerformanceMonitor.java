...
public class PerformanceMonitor {
    ...
    private static final double HIGH_RAM_THRESHOLD = 0.85; // 85%
    private static final double MEDIUM_RAM_THRESHOLD = 0.70; // 70%
    private static final double LOW_FPS_THRESHOLD = 0.75; // 75% of target

    private static PerformanceLevel currentLevel = PerformanceLevel.NORMAL;
    private static long lastLevelCheck = 0;
    private static final long LEVEL_CHECK_INTERVAL = 1000; // 1 saniye
    ...
    public static void setTargetFps(int fps) {
        ...
        LOGGER.info("Target FPS set to: {}", targetFps);
    }
    ...
    public static void tick() {
        ...
        if (elapsed >= 1_000_000_000L) {
            ...
            avgFps = (avgFps * 0.95) + (currentFps * 0.05); // Smoothing
    ...
    private static void updatePerformanceLevel() {
        ...
        if (ramUsage > HIGH_RAM_THRESHOLD || fpsRatio < 0.5) {
            currentLevel = PerformanceLevel.CRITICAL;
        } else if (ramUsage > MEDIUM_RAM_THRESHOLD || fpsRatio < LOW_FPS_THRESHOLD) {
    ...
    public static int getRecommendedChunksPerTick(int configured) {
        ...
        return Math.max(1, recommended); // En az 1
    }
}
